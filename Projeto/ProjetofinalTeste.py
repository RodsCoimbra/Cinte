...
if __name__ == '__main__':
    ...
    FS_Net_rating.add_rules(["IF (Out_BW IS Low) AND (Latency IS Low) THEN (Net_rating IS Bad)",
                             "IF (Out_BW IS Low) AND (Latency IS High) THEN (Net_rating IS Bad)",
    ...
    CLPV_D = TrapezoidFuzzySet(-1, -1, -0.75, -0.2, term="Decrease")
    CLPV_MD = TrapezoidFuzzySet(-0.8, -0.5, -0.2, 0.3, term="MDecrease")
    CLPV_MI = TrapezoidFuzzySet(-0.3, 0, 0.3, 0.8, term="MIncrease")
    CLPV_I = TrapezoidFuzzySet(0.2, 0.75, 1, 1, term="Increase")
    ...
    """ FS_PC_Load.set_variable("ProcessorLoad", df["ProcessorLoad"][12])
    FS_PC_Load.set_variable("MemoryUsage", df["MemoryUsage"][12])
    PC_Load_ = FS_PC_Load.inference()['PC_Load']
    print("\nO valores são: ", df["ProcessorLoad"][12],
          "\t", df["MemoryUsage"][12], "\t", PC_Load_) """
