...
Train_test = False   #Mudar para False no caso de querer treinar e testar em 2020-2022 e True para treinar em 2011-2019 e testar nos de 2020-2022
...
print_gen = False   #Mudar para True no caso de querer imprimir os resultados de 10 em 10 gerações


save_dados = False #Mudar para True no caso de querer guardar os dados para usar em gráficos
...
#Retorna o size do array de dados 
...
#Retorna uma flag que se True o primeiro é minimo e se False o primeiro é máximo
...
#Numa lista ordenada encontra o indice que seja maior que o valor passado
def find_next_index(Array,idx):
    next_index = bisect.bisect_right(Array, idx)
    return next_index

#Retorna o valor do close do dataframe no index passado
def df_close_value(idx):
    return df['Close'].iloc[int(idx)]

#Verifica se o index está fora do array ou se o valor do array é maior que o fim
...
def drawdown_short(inicio, fim):
    ...
    inicio_min = False                      #Flag que se True então o valor do inicio é menor que o próximo minimo
    if(flag_max == True):                   #Caso em que o primeiro é minimo
        idx_max = find_next_index(Array_max, inicio)        #Procura no Array_max o indice com valor maior que o indice do inicio
        ...
        if(df_close_value(inicio) < df_close_value(Array_min[idx_min])):    #caso o do inicio seja menor continua a testar para esse valor, 
            ...
            inicio_min = True                       #Flag a True, porque  o valor do inicio é menor que o próximo minimo
        if(inicio_min == False): 
    ...
    while True:
            if(Is_out(idx_min, Array_min, fim)):          #Condições de saida quando termina em minimo local
                ...
                #Verifica o DD atual com o maximo e incrementa os indices
                if(DD > max_drawdown):
                    max_drawdown = DD
                idx_max += 1
                idx_min_aux +=1
...
def drawdown_long(inicio, fim):
    max_drawdown = 0
    flag_max = drawdown_flag_init(inicio)
    inicio_max = False                      #Flag que se True então o valor do inicio é maior que o próximo máximo
    if(flag_max == True):                   #Caso em que o primeiro é minimo
...
def short_results(RSI_short, lls, uls, type):
    ...
    if(dados_size(RSI_short, lls, 'short') == 0):
        venda_final = True              #Casos em que nunca passa o limite inferior do short
    
    while True:
        if(flag_buy == False):          #Caso em que estamos a vender
            if (index_sell >= dados_size(RSI_short, uls, 'short')):    #Caso em que já não há mais para vender
                break
            
            else:
                #Tem uma compra antes de vender, logo passar essa compra à frente
                if (venda_final == True or dados_index(RSI_short, lls, 'short', index_buy) < dados_index(RSI_short, uls, 'short', index_sell)):  
                    
                    #Caso em que já não haja próximo buy, logo vende onde está o index_sell e compra no indice final
                    if(index_buy+1 >= dados_size(RSI_short, lls, 'short')):         
                        ...
                        buy_short = df_close_value(df[RSI_period_short].size-1) #Ultimo Valor
                        
                        #Caso de estarmos a ver o drawdown também
                        if(type == 1):          
                    ...
                    #Caso em que não terminou o array do buy, então passa para o index seguinte do buy 
                    else:                 
                        index_buy+=1  

                else:
                    #Caso em que o indice do buy é igual ao sell, logo não realiza transação
                    ...
                    sell_short_aux = dados_index(RSI_short, uls, 'short', index_sell)
                    sell_short = df_close_value(sell_short_aux)
                    flag_buy = True         #Vendeu neste index, agora precisa de comprar


        else:     #Compra
            buy_short_aux = dados_index(RSI_short, lls, 'short', index_buy)
            buy_short = df_close_value(buy_short_aux)
            flag_buy = False    #Comprou neste index, agora precisa de vender
            Roi_short = np.append(Roi_short, ROI_short(sell_short, buy_short))
    ...
    while True:
        if(flag_sell == False):      #Caso em que estamos a comprar
            if (index_buy >= dados_size(RSI_long, lll, 'long')):        #Caso em que já não há mais para comprar
                ...
                #Tem uma venda antes de comprar, logo passar essa venda à frente
                if (venda_final == True or dados_index(RSI_long, lll, 'long', index_buy) > dados_index(RSI_long, ull, 'long', index_sell)):     
                    
                    #Caso em que já não haja próximo sell, logo compra onde está o index_buy e vende no indice final
                    if(index_sell+1 >= dados_size(RSI_long, ull, 'long')):                                                               
                        index_buy_aux = dados_index(RSI_long, lll, 'long', index_buy)
                        buy_long = df_close_value(index_buy_aux)
                        sell_long = df_close_value(df[RSI_period_long].size-1)  #Ultimo Valor
                    ...
                    #Caso em que não terminou o array do sell, então passa para o index seguinte do sell
                    else:                              
                        index_sell+=1   

                #Caso em que o indice do buy é igual ao sell, logo não realiza transação
                elif(dados_index(RSI_long, lll, 'long', index_buy) == dados_index(RSI_long, ull, 'long', index_sell)): 
                    index_buy+=1
                    continue
                else:
                    index_buy_aux = dados_index(RSI_long, lll, 'long', index_buy)
                    buy_long = df_close_value(index_buy_aux)            #Compra
                    flag_sell = True        #Comprou neste index, agora precisa de vender


        else:    #Vende
            index_sell_aux = dados_index(RSI_long, ull, 'long', index_sell)
            sell_long = df_close_value(index_sell_aux)
            flag_sell = False       #Vendeu neste index, agora precisa de comprar
            Roi_long = np.append(Roi_long, ROI_long(sell_long, buy_long))
            ...
            #Procura o próximo index de compra possivel
            while (dados_index(RSI_long, lll, 'long', index_buy) < dados_index(RSI_long, ull, 'long', index_sell)): 
                    #Caso em que já não haja mais compras possiveis
...
#Função que retorna o ROI total e o drawdown máximo
...
#Guarda os indices de todos os minimos e maximos locais
def pre_processing_drawdown(df):
    Array_max = []
    Array_min = []
    flag_max = drawdown_flag_init(0)

    if (flag_max == True): #Começa com um minimo
...
#Guarda para todos os limites os locais onde o RSI transpõe esses mesmos limites, sendo que os primeiros 63 indices são do short e os restantes do long
...
def pre_processing_dados(df):
    ...
    for i in range(7,22,7):
        ...
        for j in range(0,101,5):
            index+=1            #Este index vai até 62, por isso o long come
            for idx, value in enumerate(df[RSI_period]): 
                if(df[RSI_period].size == idx+1):
                    break
                else:
                    if (value > j and df[RSI_period].iloc[idx+1] <= j): #0-62 short
...
def create_EA(nsga2):
    ...
    if nsga2:
        # Minimize (-1) or maximize (1) drawdown and ROI 
    ...
    #As variáveis que serão usadas como genes
    ...
    while g < runs:
        ...
        for mutant in offspring:
            ...
            if random.random() < MUTPB:
                ...
                if (mutant[0]>2):       #Como o randint vai de 0 a 2 nos primeiros 2 genes, temos de garantir que não passa o limite
                    mutant[0]%= 3
                if (mutant[1]>2):
                    mutant[1]%=3
        ...
        # The population is selected between parents and offsprings
        if nsga2:
        ...
        if(g%10 == 0 and print_gen):
            print("-- Generation %i --" % g)
            if(nsga2):
                print("Best individual for Maximization of ROI is: ROI->", HallofFame[0].fitness.values[0], "  DD->", HallofFame[0].fitness.values[1])
                print("Best individual for Minimization of DD is: ROI->", HallofFame[-1].fitness.values[0], "  DD->", HallofFame[-1].fitness.values[1])
            else:
                print("Best individual ROI is: ", tools.selBest(pop, 1)[0].fitness.values[0])
        if not(nsga2):
            #Guarda o melhor valor de ROI e os genes
...
if __name__ == '__main__':
    ...
    if(nsga2):
        print("--------------NSGA2 selected-------------------------")
    elif(Train_test):
        print("--------------Train and Test selected-------------------------")
    ...
    for i in path:      
        ...
        if nsga2:
            ...
            for j in range(0, runs):
                ...
                print("For the best ROI:  ROI->", HallofFame[0].fitness.values[0], "  DD->", HallofFame[0].fitness.values[1])
                print("For the best DD:  ROI->", HallofFame[-1].fitness.values[0], "  DD->" ,HallofFame[-1].fitness.values[1])
